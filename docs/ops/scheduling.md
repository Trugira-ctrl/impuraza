# Running `monitor_open_signals.py` every 5 minutes

The script is single-shot: one invocation queries the API once and exits.
An external scheduler is responsible for the "every 5 minutes" part. Two
...
**Watch it run (first run happens up to 5 min after loading; `RunAtLoad` is
...
```cron
*/5 * * * * cd /path/to/NCDs && /usr/bin/python3 scripts/monitor_open_signals.py >> data/logs/cron.stdout.log 2>&1
```
...
- Exit code: `0` success, `1` config error (bad credentials, missing
  ...
  retries (transient - the next run 5 minutes later will likely recover on
...
- **Alerting**: right now this only writes files/logs; nothing pages anyone.
  ...
  something that would re-fire every 5 minutes for the same backlog.
